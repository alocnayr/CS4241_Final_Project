      ...
      {weatherInfo && (
        <div id="weatherInfo">
          ...
          {weatherInfo.forecast.forecastday.map((day, index) => (
            <div key={index} className="dayCard" onClick={() => toggleDayDetails(index)}>
        ...
        <div className="hourlyForecastContainer">
          <div className="hourlyForecast">
            {day.hour.map((hourData, hourIndex) => (
              <div className="hourlyForecastItem" key={hourIndex}>
                <div className="hourlyTime"><strong>{formatTime(hourData.time)}</strong></div>
                <div className="hourlyIcon"><img src={hourData.condition.icon} alt="Weather Icon" /></div>
                <div className="hourlyTemp">{hourData.temp_f}°F</div>
              </div>
            ))}
          </div>
        </div>
